<template>
  <div class="min-h-screen bg-dark-900 p-8">
    <div class="max-w-4xl mx-auto">
      <h1 class="text-2xl font-bold text-claude-text dark:text-white mb-6">🔍 Diagnóstico do Caderno</h1>

      <div class="space-y-4">
        <!-- User Info -->
        <div class="bg-dark-800 border border-dark-700 rounded-claude-md p-4">
          <h2 class="text-lg font-semibold text-claude-text dark:text-white mb-2">👤 Usuário</h2>
          <pre class="text-xs text-claude-text-secondary dark:text-gray-300">{{ JSON.stringify(user, null, 2) }}</pre>
        </div>

        <!-- Test Results -->
        <div class="bg-dark-800 border border-dark-700 rounded-claude-md p-4">
          <h2 class="text-lg font-semibold text-claude-text dark:text-white mb-4">📊 Resultados dos Testes</h2>
          ...
          <div class="space-y-3">
            <div v-for="test in tests" :key="test.name" class="border-l-4 pl-3 py-2"
              ...
              <div class="flex items-center justify-between">
                <span class="text-claude-text dark:text-white font-medium">{{ test.name }}</span>
              ...
              <p class="text-sm text-claude-text-secondary dark:text-gray-400 mt-1">{{ test.message }}</p>
              <pre v-if="test.data" class="text-xs text-gray-600 dark:text-gray-500 mt-2 bg-dark-900 p-2 rounded overflow-auto max-h-40">{{ JSON.stringify(test.data, null, 2) }}</pre>
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          <button
            @click="runTests"
            class="px-4 py-2 bg-primary-500 text-claude-text dark:text-white rounded-claude-md hover:bg-primary-600 transition-colors"
            :disabled="testing"
          >
          ...
          <button
            ...
            class="px-4 py-2 bg-green-500 text-claude-text dark:text-white rounded-claude-md hover:bg-green-600 transition-colors"
            :disabled="testing"
          >
          ...
          <NuxtLink
            to="/notebook"
            class="px-4 py-2 border border-dark-600 text-claude-text-secondary dark:text-gray-400 rounded-claude-md hover:bg-dark-700 transition-colors"
          >
