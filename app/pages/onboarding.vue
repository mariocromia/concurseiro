<template>
  <div class="min-h-screen bg-gradient-to-br from-dark-900 via-dark-800 to-dark-900 flex items-center justify-center p-4">
    <div class="bg-dark-800/50 backdrop-blur-sm border border-dark-700 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden">
      <!-- Header -->
      ...
      <div class="p-8">
        ...
        <div v-if="step === 1" class="space-y-6">
          <div class="text-center">
            ...
            <h2 class="text-2xl font-bold text-white mb-2">Defina sua meta de estudo</h2>
            <p class="text-gray-400">Qual é o seu objetivo principal?</p>
          </div>
          ...
          <div class="space-y-4">
            <div>
              ...
              <input
                ...
                class="w-full px-4 py-3 bg-dark-900 border border-dark-700 text-white placeholder-gray-500 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows="3"
                class="w-full px-4 py-3 bg-dark-900 border border-dark-700 text-white placeholder-gray-500 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none"
              ></textarea>
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-300 mb-2">Data objetivo (opcional)</label>
              <input
                v-model="targetDate"
                type="date"
                class="w-full px-4 py-3 bg-dark-900 border border-dark-700 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              />
            </div>
          </div>
        </div>
        ...
        <div v-else-if="step === 2" class="space-y-6">
          <div class="text-center">
            ...
            <h2 class="text-2xl font-bold text-white mb-2">Adicione suas matérias</h2>
            <p class="text-gray-400">Quais matérias você vai estudar?</p>
          </div>
          ...
          <!-- Add Subject Form -->
          <div class="bg-dark-900/50 border border-dark-700 p-4 rounded-lg">
            <div class="flex gap-3">
              <input
                v-model="newSubject.name"
                type="text"
                placeholder="Nome da matéria"
                class="flex-1 px-3 py-2 bg-dark-900 border border-dark-700 text-white placeholder-gray-500 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                @keyup.enter="addSubject"
              />
              <select
                v-model="newSubject.color"
                class="px-3 py-2 bg-dark-900 border border-dark-700 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              >
              ...
              <button
                ...
                class="px-4 py-2 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-lg hover:from-primary-600 hover:to-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
              >
                Adicionar
              </button>
            </div>
          ...
          <div v-if="subjects.length > 0" class="space-y-3">
            <h3 class="font-medium text-gray-300">Matérias adicionadas:</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div
                v-for="(subject, index) in subjects"
                :key="subject.id || index"
                class="flex items-center justify-between p-3 border border-dark-700 rounded-lg bg-dark-900/50 hover:border-primary-500/50 transition"
              >
          ...
          <div v-else class="text-center py-8">
            <p class="text-gray-400">Nenhuma matéria adicionada ainda</p>
            <p class="text-sm text-gray-500 mt-2">Adicione pelo menos uma matéria para continuar</p>
          </div>
        </div>
        ...
        <div v-else-if="step === 3" class="text-center space-y-6">
          ...
          <p class="text-gray-400">Sua configuração inicial foi salva com sucesso.</p>

          <div class="bg-primary-500/10 border border-primary-500/30 p-6 rounded-lg">
            <h3 class="font-medium text-primary-400 mb-4">Resumo da configuração:</h3>
            <div class="text-left space-y-2">
              <p class="text-gray-300"><strong class="text-white">Meta:</strong> {{ goalName }}</p>
              <p class="text-gray-300" v-if="goalDescription"><strong class="text-white">Descrição:</strong> {{ goalDescription }}</p>
              <p class="text-gray-300" v-if="targetDate"><strong class="text-white">Data objetivo:</strong> {{ new Date(targetDate).toLocaleDateString('pt-BR') }}</p>
              <p class="text-gray-300"><strong class="text-white">Matérias:</strong> {{ subjects.length }} adicionadas</p>
            </div>
          </div>

          <button
            @click="$router.push('/dashboard')"
            class="w-full px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-lg hover:from-primary-600 hover:to-primary-700 transition-colors font-semibold"
          >
            Começar a estudar 🚀
          </button>
        </div>
        ...
        <div v-if="step < 3" class="flex justify-between mt-8 pt-6 border-t border-dark-700">
          <button
            v-if="step > 1"
            @click="step--"
            class="px-4 py-2 border border-dark-700 rounded-lg text-gray-300 hover:bg-dark-700 transition-colors"
          >
            Voltar
          </button>
          <div v-else></div>
          ...
          <button
            ...
            class="px-6 py-2 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-lg hover:from-primary-600 hover:to-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
