-- ============================================
-- SCHEMA DO BANCO DE DADOS - CONCURSEIRO
-- VERSÃO SAFE (DROP IF EXISTS)
-- ============================================
...
DROP POLICY IF EXISTS "Usuários podem ver seus anexos" ON storage.objects;
...
DROP TABLE IF EXISTS public.notebook_pages CASCADE;
DROP TABLE IF EXISTS public.notebook_sections CASCADE;
...
-- Políticas notebook_pages
CREATE POLICY "Usuários podem ver páginas das suas seções" ON public.notebook_pages FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.notebook_sections JOIN public.notebooks ON notebooks.id = notebook_sections.notebook_id WHERE notebook_sections.id = notebook_pages.section_id AND notebooks.user_id = auth.uid()));
CREATE POLICY "Usuários podem inserir páginas nas suas seções" ON public.notebook_pages FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.notebook_sections JOIN public.notebooks ON notebooks.id = notebook_sections.notebook_id WHERE notebook_sections.id = notebook_pages.section_id AND notebooks.user_id = auth.uid()));
...
-- Políticas page_attachments
CREATE POLICY "Usuários podem ver anexos das suas páginas" ON public.page_attachments FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.notebook_pages JOIN public.notebook_sections ON notebook_sections.id = notebook_pages.section_id JOIN public.notebooks ON notebooks.id = notebook_sections.notebook_id WHERE notebook_pages.id = page_attachments.page_id AND notebooks.user_id = auth.uid()));
CREATE POLICY "Usuários podem inserir anexos nas suas páginas" ON public.page_attachments FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.notebook_pages JOIN public.notebook_sections ON notebook_sections.id = notebook_pages.section_id JOIN public.notebooks ON notebooks.id = notebook_sections.notebook_id WHERE notebook_pages.id = page_attachments.page_id AND notebooks.user_id = auth.uid()));
...
DELETE FROM storage.buckets WHERE id IN ('avatars', 'page-attachments');
